<x-app-layout>
    <x-slot name="breadcrumbs">posts.create</x-slot>

    <form action="{{ route('admin.blog.posts.create') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="flex flex-col lg:flex-row">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <div class="flex flex-col col-span-2">
                        <x-form-input-text :messages="$errors->get('title')" type="text" id="title" name="title"
                            value="{{ old('title') }}" label="Judul" placeholder="Ex: Lorem ipsum dolor sit amet">
                        </x-form-input-text>
                    </div>
                    <div class="flex flex-col col-span-2">
                        <x-form-input-textarea :messages="$errors->get('excerpt')" id="excerpt" name="excerpt"
                            value="{{ old('excerpt') }}" label="Deskripsi Singkat" rows="2">
            ...
            <x-card-container class="w-full lg:w-[480px]">
                ...
                <div class="flex flex-col gap-2">
                    <x-form-input-text :messages="$errors->get('post_date')" type="text" id="post-date" name="post_date"
                        label="Tanggal Posting" class="hidden">
                    ...
                    <x-form-input-checkbox-helper :messages="$errors->get('post_comment_status')" label="Izinkan
                        Komentar"
                        value="true" id="post-comment-status" name="post_comment_status"
    ...
    @push('js')
        <!-- Scripts -->
        <script src="{{ asset('assets/js/module/blog/post/create.js') }}"></script>
    @endpush
</x-app-layout>

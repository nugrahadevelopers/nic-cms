<x-app-layout>
    <x-slot name="breadcrumbs">users.edit</x-slot>
    <x-slot name="breadcrumbsData">{{ $user->name }}</x-slot>
        ...
        <form action="{{ route('admin.users.edit', $user) }}" method="post">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                <x-form-input-text :messages="$errors->get('name')" type="text" id="name" name="name"
                    value="{{ old('name', $user->name) }}" label="Nama" placeholder="Ex: Reno Nugraha">
                </x-form-input-text>
                <x-form-input-text :messages="$errors->get('email')" type="email" id="email" name="email"
                    value="{{ old('email', $user->email) }}" label="Email" placeholder="Ex: reno@rn.com">
                </x-form-input-text>
                ...
                <x-form-select-input :messages="$errors->get('roles')" id="roles-select" name="roles[]" label="Pilih Role"
                    ...
                    data-find-url="{{ route('admin.users.get-selected-role', $user) }}">
                </x-form-select-input>
            </div>
    ...
    @push('js')
        <!-- Scripts -->
        <script src="{{ asset('assets/js/panel/admin/user/edit.js') }}"></script>
    @endpush
</x-app-layout>
